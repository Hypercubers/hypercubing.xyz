...
These properties are redundant: suppose we call these $X, g, Y$ respectively. Then $Y = \{g \cdot x \mid x \in X\}$, or simply $Y = g \cdot X$; and likewise and $X = g^{-1} \cdot Y$.
...
- If the piece is currently incident with $f$, it gets acted on by $r$, sending $(X, g, Y) \mapsto (X, rg, r \cdot Y)$.
...
Next consider a group acting on this set, generated by the 4-cycles $F, U, B, D$, and $R, F, L, B$. More precisely, we mean the subgroup $G$ of $S_\Gamma$ generated by $(FUBD)$ and $(RFLB)$. We will call this the *grip group*. This group and its action are isomorphic to the rotations of the cube and their action on the 6 faces we defined above.
...
$$r \cdot (g, Y) = (rg, r \cdot Y),$$

that is, the set of active grips follows the piece. As before for a piece $(g, Y)$, we can recover the set of grips when solved as $X = g^{-1} \cdot Y$. We thus define two pieces $(g, Y)$ and $(g', Y')$ to be identical if:

$$g^{-1} \cdot Y = g'^{-1} \cdot Y'.$$

The attitude of a piece can be decomposed into two components: the *orientation* of the piece, which is an element of the [stabilizer subgroup](https://en.wikipedia.org/wiki/Group_action#Orbits_and_stabilizers) $\operatorname{Stab}(Y)$; and the *position*, which is the set of active grips $Y$ itself. By the [orbit-stabilizer theorem](https://en.wikipedia.org/wiki/Group_action#Orbit-stabilizer_theorem) we can identify the orbit of $G \cdot Y$ (the possible positions of this piece) with the cosets in $G / \operatorname{Stab}(Y)$.

For example, $((UR)(LD)(FB), \{R, U\})$ is an abstract description of the white-red edge in its solved position, but wrong orientation.

Note that the groups $\operatorname{Stab}(Y)$ and $\operatorname{Stab}(X) = \operatorname{Stab}(g^{-1} \cdot Y)$ are isomorphic, and in general we have a [fiber bundle](https://en.wikipedia.org/wiki/Fiber_bundle):
