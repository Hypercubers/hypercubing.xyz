...
Define a *piece* to have two things: its *attitude* $g$, which is an element of the grip group $G$, and a set $X$ of *solved grips*. A piece also has a set of *active grips*, which is defined to be $Y = X \cdot g$. The grip group acts on pieces as follows:

$$ (X, g) \cdot r = (X, gr).$$

The set of active grips of $(X, g) \cdot r$ is therefore $Y \cdot r$, so it follows the piece. We define two pieces $(X, g)$ and $(X, g')$ to be identical if $X = X'$.
...
For example, $(\{R, U\}, (U,R)(L,D)(F,B))$ is an abstract description of the up-front edge in its solved position, but wrong orientation.

Note that the groups $\operatorname{Stab}(X)$ and $\operatorname{Stab}(Y) = \operatorname{Stab}(X \cdot g)$ are isomorphic, and in general we have a [fiber bundle](https://en.wikipedia.org/wiki/Fiber_bundle):
...
Within a puzzle whose pieces have solved twists in the set $S$, each twist can be interpreted as an element of the direct product group $G^S$, where the copy of $G$ at a certain index $X \in S$ corresponds to the attitude of the piece with solved grips $X$. Thus, the set of twists generates a subgroup of $G^S$.
