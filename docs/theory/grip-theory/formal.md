...
Next, define a *twist* to be a pair of: a grip $f$ we twist around, and an element of $\operatorname{Stab}(f)$, the *transformation*. Equivalently, we can choose an arbitrary transformation $r \in G$ first, and then a grip $f$ from the set $\Gamma^r$ of fixed points of $r$. As before, the action of a twist $(f, r)$ on a piece is:
...
For instance, applying the twist with grip $U$ and transformation $(R,F,L,B)$, which fixes $U$, to the piece from above, we get a new attitude
...
The set of all positions our piece can move to are all sets of two grips except $\{R, L\}$, $\{U, D\}$, and $\{F, B\}$. These exactly correspond to the edge positions of the 3×3×3. An edge piece is only affected by a twist if one of its colored faces is on the face corresponding to the grip of the twist, and after the twist, the positions of its colored faces moves. This is exactly the behavior of the active grips. Therefore, the piece above is an edge piece.
...
This defines a [group homomorphism](https://en.wikipedia.org/wiki/Group_homomorphism) $\operatorname{twist}_f : \operatorname{Stab}(f) \to G \wr P$. To be able to talk about a sequence of twists on different grips, we can define the *twist group* as the [free product (coproduct)](https://en.wikipedia.org/wiki/Free_product) of the groups of twists on each grip:
