...
- A **to-adjacent** cut is a cut that intersects the adjacent axis line at the surface of the puzzle. An example is the [Pyraminx Crystal](https://twistypuzzles.com/app/museum/museum_showitem.php?pkey=652) or the [Dino Cube](https://twistypuzzles.com/app/museum/museum_showitem.php?pkey=605). A to-adjacent cut is exactly the depth required to not have axis pieces (pieces that turn with exactly one grip).
